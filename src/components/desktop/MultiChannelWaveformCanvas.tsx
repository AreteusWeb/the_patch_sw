...
function resolveScale(
  ...
  if (!ch.ecgScale) {
    ...
    if (dataMin === dataMax) return { min: ch.min, max: ch.max };
    const padding = (dataMax - dataMin) * 0.1 || 0.01;
    return { min: dataMin - padding, max: dataMax + padding };
  }

  // ECG without paper grid — fit visible data, clamped to a sane ECG window
  let dataMin = Infinity;
  let dataMax = -Infinity;
  for (const v of slice) {
    if (v < dataMin) dataMin = v;
  ...
  return {
    min: Math.min(ch.min, dataMin - padding),
    max: Math.max(ch.max, dataMax + padding),
  };
}
...
const MultiChannelWaveformCanvas: React.FC<MultiChannelWaveformCanvasProps> = ({
  ...
  const draw = useCallback(() => {
    ...
    channels.forEach((ch, laneIdx) => {
      ...
      const { min, max } = resolveScale(ch, slice, drawableH, pxPerMm, gain, usePaper);
      const range = max - min || 1;

      ctx.save();
      ctx.beginPath();
      ctx.rect(traceLeft, traceTop, traceWidth, drawableH);
      ctx.clip();
      ...
      for (let i = 0; i < slice.length; i++) {
        ...
        const y = traceTop + (1 - normalized) * drawableH;
  ...
  const resolvedHeight = usePaper && fill
  ...
  return (
    <div
      ref={wrapRef}
      className={`relative w-full overflow-hidden bg-[#0a0a0f]${fill && !usePaper ? ' h-full' : ''}`}
      style={
        resolvedHeight != null
