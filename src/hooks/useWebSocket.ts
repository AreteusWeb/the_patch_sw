...
const SIM_PARAMS: Record<SimMode, { hr: number; resp: number; temp: number; spo2: number; sys: number; dia: number }> = {
    ...
    tachycardia: { hr: 135, resp: 20, temp: 36.8, spo2: 96, sys: 135, dia: 85 },
    bradycardia: { hr: 42, resp: 14, temp: 36.5, spo2: 97, sys: 105, dia: 65 },
    spo2drop: { hr: 80, resp: 22, temp: 36.6, spo2: 85, sys: 125, dia: 80 },
    fever: { hr: 95, resp: 20, temp: 39.2, spo2: 97, sys: 128, dia: 82 },
};
    ...
    useEffect(() => {
        ...
        const startSim = () => {
            ...
            simRef.current = setInterval(() => {
                simTime.current += 0.1;
                handlePacket(buildSimPacket(simTime.current, simulationMode as SimMode));
            }, 100);
        };
        // FIN SIMULADOR
